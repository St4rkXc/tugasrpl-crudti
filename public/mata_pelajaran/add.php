...
<body class="bg-gray-50 min-h-screen flex items-center justify-center">
    <main class="w-full max-w-2xl bg-white shadow-md rounded-lg p-8">
        <header class="mb-6">
            <h1 class="text-2xl font-semibold text-gray-800">Tambah Data Mapel</h1>
            <p class="text-sm text-gray-500">Isi form berikut untuk menambahkan mapel baru ke dalam database.</p>
        </header>

        <form method="post" class="space-y-6">
            <div class="grid grid-cols-1 gap-4">
                <label class="block">
                    <span class="text-sm font-medium text-gray-700">Nama Mapel</span>
            ...
            <div class="flex items-center justify-between">
                ...
                <button type="submit" name="simpan"
                    class="inline-flex items-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-200">
                    Simpan
                </button>
            </div>
        </form>
        ...
        if (isset($_POST['simpan'])) {
            $stmt = $koneksi->prepare("INSERT INTO mata_pelajaran (nama_mapel, kelas, guru_pengajar) VALUES (:nama_mapel, :kelas, :guru_pengajar)");
            $stmt->execute([
